...
function tree(schema, options) {
  ...
  schema.pre('remove', function preRemove(next) {
    ...
    if (onDelete == 'DELETE') {
      ...
      this.collection.find({ parent: previousParent }, { _id: 1 }, function (err, cursor) {
        ...
        function streamOnClose(err) {
          ...
          self.collection.find({ path: { $regex: previousParent + pathSeparatorRegex} }, { path: 1 }, function (err, cursor) {
  ...
  schema.statics.getChildrenTree = function getChildrenTree(root, args, next) {
    ...
    return getModel(root).find(filters, fields, options, function (err, results) {
